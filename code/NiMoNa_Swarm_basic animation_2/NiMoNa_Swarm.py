...
#r = [0,0.5,1,1.5]
r = [0,2,5,10] 
factor = [2.5,1] # scaling factors for repulsion and attraction
def sigma_swarm(x):
...
def plotOrSave(t, axis):
    ...
    ax.set_aspect(1)
    ax.set_title(f'radii: repulsion:{r[0]}-{r[1]}, attraction:{r[2]}-{r[3]}\nfactors: rep. {factor[0]}, attr. {factor[1]}   timestep {t}/{n}')
    plt.scatter(p[t,:,0], p[t,:,1], color='b')#, c=np.arange(k))#, cmap='tab10')
...
#    encoded_instance = jsonpickle.encode(p[0].tolist())
...
draw_rectangle = np.array([0.,1.,0.,1.])
#print(p[0])
...
model_xmin_xmax_ymin_ymax = [0,5,0,5]
model_xmin_xmax_ymin_ymax = [-2,3,-2,3]
