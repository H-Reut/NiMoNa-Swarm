...
def visualize_2d_ffmpeg(pos, vel=None, xmin=-5, xmax=5, ymin=-5, ymax=5, save_anim=None, T_max=0, dt=0, inverse_arrow_scale_factor = 1, img_save_path = 'H:/dump/', fps=30, delete_frames=True, show_plots=False, open_anim=True, draw_rectangle=[0.,0.,0.,0.], origin_color=None):
    '''
    ...
    assert fps > 0, 'fps needs to be greater than 0'
    ...
    for t in range(n_steps):
        ...
        if t!=0:
            ...
            print(f'generating images\t{str(t).rjust(len(str(n_steps-1)))} / {n_steps}\t({t/n_steps:.0%})\tTime left ca. {eta_left/60:.1f} minutes\tTotal time ca. {eta_total/60:.1f} minutes', end="\r")
        ...
        agents_on_screen = np.sum((np.all(pos[t,:] >= mins, 1) & np.all(pos[t,:] <= maxs, 1)))
    ...
    if open_anim:
        print(f'Opening file:\t{img_save_path+f'anim_{file_identifier}.mov'}')
