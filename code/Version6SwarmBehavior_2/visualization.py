...
def visualize_2d(pos, vel=None, xmin=-5, xmax=5, ymin=-5, ymax=5, save_anim=False, filename='', interval=0., format='.gif', draw_rectangle=[0., 0., 0., 0.], fps=10, titleAppendix=''):
    '''
    ...
    ax.set_title(f'Timestep {str(0).rjust(len(str(k_steps-1)))}/{k_steps-1}'+titleAppendix)
    # rectangle
    ...
    def anim(frame):
        ...
        ax.set_title(f'Timestep{str(frame).rjust(len(str(k_steps-1)))}/{k_steps-1}'+titleAppendix)
    ...
    if save_anim:
        ...
        if not filename == 'no' and not filename == '':
            if format == '.gif':
                anim_created.save(filename + '.gif', writer=PillowWriter(fps=fps))
            if format == '.mp4':
                anim_created.save(filename + '.mp4', writer=FFMpegWriter(fps=50, bitrate=10000))
            else:
                assert format[0] == '.'
