...
def plotOneFrame(fig, ax, pos, vel=None, timestep=0, numtimesteps=0, xmin=-5, xmax=5, ymin=-5, ymax=5, zmin=-5, zmax=5, T_max=0, dt=0, inverse_arrow_scale_factor=1, draw_rectangle=[0., 0., 0., 0.], origin_color=None, colors=None, subplotName=None):
    assert timestep <= numtimesteps
    assert (pos.ndim == 2), 'Positions need to have the shape (number of individuals) x 2'
    assert (pos.shape[1] == dim), 'Positions need to have the shape (number of individuals) x 2'
    if vel is not None:
...
def visualize_2d_ffmpeg_multi(poss, vels=None, axis=[-1., 1.,-1., 1.], T_max=0, dt=0, img_save_path='H:/dump/', fps=30, delete_frames=True, show_plots=False, open_anim=True, inverse_arrow_scale_factor=1, draw_rectangle=[0., 0., 0., 0.], origin_color=None, colors=None, file_identifier="", plottitles=None, res=(19.20, 10.80), order=None):
    '''
    ...
    k_frames, N_agents = poss.shape[-3:-1]
    ...
    for t in order:
        ...
        for img_y, img_x in np.ndindex(plotgrid):
            ...
            zmin=axis[img_x][img_y][4]
            zmax=axis[img_x][img_y][5]
            ...
            plotOneFrame(fig, ax, poss[img_y, img_x, t, ...], vel=None, timestep=t, numtimesteps=k_frames, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, zmin=zmin, zmax=zmax, T_max=T_max, dt=dt, inverse_arrow_scale_factor=inverse_arrow_scale_factor, draw_rectangle=draw_rectangle, origin_color=origin_color, colors=colors, subplotName=subplottitle)
