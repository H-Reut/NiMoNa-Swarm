...
# Model parameters
k_agents = 40#250
n_timesteps = 500#5000
T = 5#50
fps = 60
dim = 2
regimeColors=['gray', 'b', 'k' if dim == 3 else 'w', 'r', 'm', 'y', 'g', 'c']

#                     [ a ,  b , C_a, C_r, l_a, l_r,  k,    n,  T,  name ]
...
# Visualize as gif
for params in paperparams:
    for edge in [1.0, 0.2, 5.0]:#0.3, 0.6, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0]:
        [ a ,  b , C_a, C_r, l_a, l_r,  k_agents,   n_timesteps,  T, animname] = params
        k_agents = 40#250
        duration = n_timesteps // 50
        filename = f'anim/{k_agents}ag {animname} {edge:.1f}ax {duration:d}s'
        print(filename)
        ...
        print("\tcreating .mp4")
        visualize_2d(x[:,:,0,:], vel=None, xmin=-edge, xmax=edge, ymin=-edge, ymax=edge, save_anim=True, filename=filename, interval=20, format='.mp4')
