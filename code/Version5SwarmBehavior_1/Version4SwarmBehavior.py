...
def morse(t, v_t, p_t):
    # General movements:
    w = (α - β * np.square(np.linalg.norm(v_t[:], axis=-1, keepdims=True))) * v_t[:]
    for i in range(N):
        # Movements caused by other agents.
        diffs = p_t[i]-p_t[:] # for i-th entry we have: diffs[i]=0.0
        norms = np.linalg.norm(diffs, axis=-1) # for i-th entry we have norms[i]=0.0
        norms[i] = np.inf
        factors = (C_r/l_r * np.exp(-norms/l_r) - C_a/l_a * np.exp(-norms/l_a)) / norms # for i-th entry we have: factors[i]=0.0
        w[i] += factors @ diffs # The effect of agent i on itself is ignored because (factors@diffs)[i]=[0.0, 0.0]
    return w
...
xs = np.zeros((len(ls), len(Cs), k+1, N, 2, dim))
print(xs.shape)
p_0 = -0.5*np.ones((N, dim)) + np.random.rand(N, dim)# *1 
...
for img_y in range(len(ls)):
    for img_x in range(len(Cs)):
        ...
        x = np.zeros((k+1, N, 2, dim))  # timesteps; agents; pos,vel; x,y
