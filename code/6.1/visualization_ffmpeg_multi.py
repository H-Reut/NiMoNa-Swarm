...
def plotOnePlot(fig, ax, pos, vel=None, timestep=0, numtimesteps=0, axis=np.array([[-1., -1., -1.], [1., 1., 1.]]), T_max=0, dt=0, inverse_arrow_scale_factor=1, draw_rectangle=[0., 0., 0., 0.], origin_color=None, colors=None, subplotName=None, plotColor='k'):
    assert timestep <= numtimesteps
    assert (pos.ndim == 2), 'Positions need to have the shape (number of individuals) x 2'
    dim = pos.shape[-1]
    ...
    xmin = axis[0,0]
    xmax = axis[1,0]
    assert xmin < xmax, f'xmin({xmin}) needs to be smaller than xmax({xmax})'
    ymin = axis[0,1]
    ymax = axis[1,1]
    assert ymin < ymax, f'ymin({ymin}) needs to be smaller than ymax({ymax})'
    if dim >= 3:
        zmin = axis[0,2]
        zmax = axis[1,2]
        assert zmin < zmax, f'zmin({zmin}) needs to be smaller than zmax({zmax})'
    ...
    if dim == 2:
        ...
        ax.set(xlim=(xmin, xmax), ylim=(ymin, ymax), zlim=(zmin, zmax))  # constant reference frame
    ax.set_aspect('equal')  # spacing the same for x and y direction
    ...
    # Plotting of data (and optional velocity, origin, rectangle)
    if draw_rectangle != [0., 0., 0., 0.] and dim == 2:
        ax.vlines(x=draw_rectangle[0:2],
                    ymin=draw_rectangle[2], ymax=draw_rectangle[3],
                    color='gray', linestyle='--', linewidth=1)
        ax.hlines(y=draw_rectangle[2:4],
    ...
    if origin_color is not None:
        if dim == 2:
            ...
            ax.scatter(0., 0., 0., c=origin_color, marker='+')
    if vel is not None:
        ax.quiver(pos[:, 0], pos[:, 1], pos[:, 2], vel[:, 0], vel[:, 1],
                    pivot='tail', width=0.002, angles='xy', scale_units='xy', scale=inverse_arrow_scale_factor, color='#c0c0c0')
    if colors is None:
        if dim == 2:
            ax.scatter(pos[:, 0], pos[:, 1],            c=colormap, cmap='tab10', marker=".")
        elif dim == 3:
            ax.scatter(pos[:, 0], pos[:, 1], pos[:, 2], c=colormap, cmap='tab10', marker=".")
    else:
        if dim == 2:
            ax.scatter(pos[:, 0], pos[:, 1],            color=colors[timestep])
        elif dim == 3:
            ax.scatter(pos[:, 0], pos[:, 1], pos[:, 2], color=colors[timestep])




def visualize_2d_ffmpeg_multi(poss, vels=None, axes=[-1., 1.,-1., 1.], borderColors=None, T_max=0, dt=0, img_save_path='H:/dump/', fps=30, delete_frames=True, show_plots=False, open_anim=True, inverse_arrow_scale_factor=1, draw_rectangle=[0., 0., 0., 0.], origin_color=None, colors=None, file_identifier="", plottitles=None, res=(19.20, 10.80), order=None):
    '''
    ...
    k_frames, N_agents, dim = poss.shape[-3:]
    print(k_frames, N_agents, dim)
    ...
    assert fps >= 1
    duration_m, duration_s = divmod(k_frames/fps, 60)
    print(f'Animation Parameters:\n\tfps = {fps}\n\tdur = {duration_m:2.0f}:{duration_s:02.0f}')

    #print(axis)

    if order==None:
        order=[t for t in range(k_frames)]
    print(len(order), k_frames)
    assert len(order) == k_frames

    print(len(order), k_frames, N_agents, poss.shape)
    assert len(order) == k_frames
    ...
    for t in order:
        ...
        if progress_counter != 0:
            ...
            eta_total = (time_now - time_start) * k_frames/progress_counter
            eta_left = eta_total - time_now + time_start
            print(
                f'generating images\t{str(progress_counter).rjust(len(str(k_frames-1)))} / {k_frames}\t({progress_counter/k_frames:.0%})\tTime: passed {time_passed/60:.1f} min,  remaining {eta_left/60:.1f} min,  total {eta_total/60:.1f} min', end="\r")
        progress_counter += 1
        ...
        #Setting suptitle (super title)
        suptitle = f'Timestep{str(t).rjust(len(str(k_frames-1)))}/{k_frames-1}'
        ...
        suptitle += f'  {N_agents} agents'
        fig.suptitle(suptitle)

        for img_y, img_x in np.ndindex(plotgrid):

            projection = 'rectilinear'
            if dim == 3:
                ax = fig.add_subplot(plotgrid[0], plotgrid[1], plotgrid[0]*img_y+img_y+img_x+1, projection='3d')
            if dim == 2:
                ax = axs[img_y, img_x]
            #axs[img_y, img_x].set_xticks([])
            
            # Setting subplot title
            subplottitle = plottitles[img_y][img_x]
            

            # getting eternal or time-specialized axis
            axis = axes[img_x][img_y]
            if axis.ndim == 3:
                if axis.shape[0] == 1:
                    axis = axis[0] # eternal axis
                else:
                    axis = axis[t] # time-specialized axis
            plotColor = 'k'
            if borderColors is not None:
                plotColor = borderColors[img_y][img_x]
            #print(borderColors, plotColor)
            plotOnePlot(fig, ax, poss[img_y, img_x, t, ...], vel=None, timestep=t, numtimesteps=k_frames, axis=axis, T_max=T_max, dt=dt, inverse_arrow_scale_factor=inverse_arrow_scale_factor, draw_rectangle=draw_rectangle, origin_color=origin_color, colors=colors, subplotName=subplottitle, plotColor=plotColor)
            ...
            ymax=axis[1][1]
            #print(f'{xmin}\n{xmax}\n{ymin}\n{ymax}')
            #plotOneFrame(fig, axs[img_y, img_x], poss[img_y, img_x, t, ...], vel=None, timestep=t, numtimesteps=k_frames, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, T_max=T_max, dt=dt, inverse_arrow_scale_factor=inverse_arrow_scale_factor, draw_rectangle=draw_rectangle, origin_color=origin_color, colors=colors, subplotName=subplottitle)
