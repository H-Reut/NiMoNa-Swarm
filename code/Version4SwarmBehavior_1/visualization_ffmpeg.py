...
def visualize_2d_ffmpeg(pos, vel=None, axis=[-1,1,-1,1], T_max=0.0, dt=0, img_save_path='H:/dump/', fps=30, delete_frames=True, show_plots=False, open_anim=True, inverse_arrow_scale_factor=1, draw_rectangle=[0., 0., 0., 0.], origin_color=None, colors=None, file_identifier=""):
    '''
    ...
    assert fps >= 1
    duration_m, duration_s = divmod(n_steps/fps, 60)
    ...
    for t in range(n_steps):
        ...
        if t != 0:
            ...
            print(
                f'generating images\t{str(t).rjust(len(str(n_steps-1)))} / {n_steps}\t({t/n_steps:.0%})\tTime: passed {time_passed/60:.1f} min,  remaining {eta_left/60:.1f} min,  total {eta_total/60:.1f} min', end="\r")
        ...
        if colors is None:
            ...
            plt.scatter(pos[t, :, 0], pos[t, :, 1], color=colors[t])
